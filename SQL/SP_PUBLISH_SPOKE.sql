...
FROM (
    WITH CS AS (select database_name, schema_name, table_name, notes, presigned_url 
            FROM $DATAPROVIDER_DB.$DATAPROVIDER_SCHEMA.CONTROL_SPOKE)           
...
execute immediate ('insert into ckan_log select current_timestamp(),package_id,table_name from published_spoke BEFORE(STATEMENT => ''' || $LQID::string || ''') WHERE status = ''PACKAGE CREATED''');
